...
# only mean is computed
def dicentered_k_adjacency_2(A, k, with_self=False, self_factor=1):
    assert isinstance(A, np.ndarray)
    I = np.eye(len(A), dtype=A.dtype)
    if k == 0:
        return I
        ...
        Ak = np.mean(np.stack([np.minimum(np.linalg.matrix_power(A + I, k), 1) -
                                                                                np.minimum(
                                                                                            np.mean(
                                                                                                    np.stack(
                                                                                                            [np.linalg.matrix_power(A + I, p) for p in range(k)],
...
# only mean is computed
def dicentered_k_adjacency_3(A, k, with_self=False, self_factor=1):
    assert isinstance(A, np.ndarray)
    I = np.eye(len(A), dtype=A.dtype)
    if k == 0:
        return I
        ...
        Ak = np.mean(np.stack([np.minimum(np.linalg.matrix_power(A + I, k), 1) -
                               np.minimum(
                                   np.mean(
                                       np.stack(
                                           # [np.linalg.matrix_power(A + I, p) for p in range(k)],
                                           [np.minimum(np.linalg.matrix_power(A + I, p), 1) for p in range(k)],
